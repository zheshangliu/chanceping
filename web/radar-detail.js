...
(function () {
  ...
  async function runRadar(radarId) {
    ...
    if (resultList) resultList.innerHTML = '<p class="placeholder">盯机会正在搜索机会并整理证据，请稍候...</p>';

    try {
      const status = document.getElementById("radar-detail-rerun-status");
      if (status) status.innerHTML = '<span class="rerun-status-running">盯机会正在重新盯机会</span>';
      ...
      if (resultList) {
        resultList.insertAdjacentHTML("afterbegin", '<p class="placeholder">盯机会正在生成报告...</p>');
      }
      if (status) status.innerHTML = '<span class="rerun-status-running">盯机会正在生成报告</span>';
