...
const NEGATIVE_TERMS = ["纯摄影", "摄影比赛", "摄影大赛", "摄影竞赛", "普通建筑竞赛", "汽车设计", "机械设计", "程序设计", "算法比赛", "纯ui", "纯 UI", "软件界面"];
...
export function classifyV2RadarRelevance(title: string, summary: string, sourceCategory = ""): { relevance: V2RadarRelevance; tags: string[] } {
  ...
  const value = `${content} ${sourceCategory}`;
  const contentStrong = hits(content, STRONG_TERMS);
  const categoryStrong = hits(sourceCategory, STRONG_TERMS);
  const generic = hits(value, GENERIC_TERMS);
  const negative = hits(content, NEGATIVE_TERMS);
  if (negative.length > 0 && contentStrong.length === 0) return { relevance: "IRRELEVANT", tags: [...new Set([...negative, ...generic])] };
  const strong = [...new Set([...contentStrong, ...categoryStrong])];
