...
const DIRECTION_RULES: Array<[V2OpportunityDirection, RegExp]> = [
  ["aigc_digital", /aigc|生成式\s*ai|人工智能创作|ai创作|数字创作/u],
  ["ich_innovation", /非遗|非物质文化遗产|传统文化|传统技艺|传承/u],
  ["cultural_creative", /文创|文化创意|城市礼物|地方礼物|伴手礼|国潮|文化ip|文博文旅/u],
  ["craft_arts", /工艺美术|手工艺|craft|craftsmanship|artisan|handmade|ceramic|陶瓷|刺绣|织染|漆艺|竹编|木雕|珠宝|jewellery|jewelry/u],
  ["museum_tourism", /博物馆|文物|文旅|文化旅游|旅游商品/u],
  ["integrated_cultural_design", /综合文化|综合设计|文化设计|文化艺术/u],
];

const WORK_FORMAT_RULES: Array<[V2WorkFormat, RegExp]> = [
  ["material_craft", /实物工艺|材料工艺|陶瓷|陶艺|木雕|竹编|刺绣|织染|漆艺|金工/u],
  ["product_design", /产品设计|文创产品|伴手礼|旅游商品|product design/u],
  ["graphic_ip", /平面|插画|视觉|海报|ip形象|吉祥物|logo|graphic|illustration/u],
  ["packaging", /包装设计|包装|packaging/u],
  ["fashion_jewellery", /服饰|首饰|珠宝|服装|jewellery|jewelry|fashion/u],
  ["video_animation", /视频|动画|短片|影像|video|animation|film/u],
  ["interaction_game", /交互|互动|游戏|interaction|game|体验设计/u],
  ["mixed_media", /综合媒介|综合材料|mixed media|跨媒介/u],
];
...
export function classifyV2Dimensions(title: string, summary: string, category: string): {
  ...
  const eventMatch = value.match(/(?:举办地|活动地点|展出地点|地点|地址)\s*[:：]?\s*([^，。；;\n]{2,32})/u);
  const scope: V2ParticipationScope = /面向全球|全球征集|全球可投|international open/u.test(value)
    ? "global"
    : /面向全国|全国征集|全国可投|全国范围/u.test(value)
      ? "nationwide"
      : /限(?:于|定)?[^，。；;\n]{2,20}(?:地区|省|市|境内)/u.test(value) ? "regional" : "unspecified";
  const mode: V2ParticipationMode = /线上提交|在线提交|网上报名|online submission|online application/u.test(value)
    ? "online"
    : /实物寄送|邮寄作品|需到场|现场提交|in person|onsite/u.test(value)
      ? (/需到场|现场提交|in person|onsite/u.test(value) ? "onsite" : "physical")
      : "unspecified";
