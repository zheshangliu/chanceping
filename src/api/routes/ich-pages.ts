...
function listPage(result: IchQueryResult, history: boolean): string {
  ...
  const href = (patch: Record<string, string>) => { const next = new URLSearchParams(queryParams); Object.entries(patch).forEach(([key, value]) => next.set(key, value)); next.delete("page"); return `/ich?${next.toString()}`; };
  ...
  return `<main><section class="ich-hero"><div class="ich-hero-copy"><p class="ich-kicker">ChancePing · 纸本地域目录</p><h1>${heading}</h1><p>${intro}</p><div class="ich-meta"><span>最近更新：${escapeHtml(result.last_updated_at || "持续更新中")}</span><span>当前机会：${result.total} 条</span></div></div></section><form class="ich-search" method="get" action="/ich"><input name="q" value="${escapeHtml(result.filters.q)}" placeholder="搜索机会名称、主办方、关键词（如：设计大赛、采购、资助）" aria-label="搜索非遗机会"><button type="submit">搜索</button></form><div class="ich-filters"><div class="ich-category-row">${categories.map(([key, label, hint]) => `<a class="ich-filter-button${result.filters.category === key ? " is-active" : ""}" href="${href({category:key})}">${label}<small>${hint}</small></a>`).join("")}</div><div class="ich-filter-line"><span>地区索引：</span>${regions.map(([key,label]) => `<a class="${result.filters.region === key ? "is-active" : ""}" href="${href({region:key})}">${label}</a>`).join("")}</div><div class="ich-filter-line"><span>状态：</span>${statuses.map(([key,label]) => `<a class="${result.filters.status === key ? "is-active" : ""}" href="${href({status:key})}">${label}</a>`).join("")}<a class="${history ? "is-active" : ""}" href="/ich/history">历史机会</a><select class="ich-sort" name="sort" aria-label="排序"><option value="default" ${result.filters.sort === "default" ? "selected" : ""}>排序：截止时间（近→远）</option><option value="newest" ${result.filters.sort === "newest" ? "selected" : ""}>排序：最新收录</option></select></div></div><div class="ich-summary"><span>已选条件：　地区：${result.filters.region}　·　状态：${history ? "历史" : result.filters.status}</span><a href="/ich">清空筛选</a></div>${content}<div class="ich-pagination">${pagination || "<span>暂无分页</span>"}</div><div class="ich-lower"><section><h2>来源与可信度</h2><p>我们优先从政府官网、主办方官网和官方报名页整理机会，线索会标明核验状态。申请前请回到官方来源复核。</p></section><section><h2>持续发现</h2><p>机会覆盖赛事、展销、采购、渠道、资助与国际交流，持续更新中。</p></section></div></main>`;
}
