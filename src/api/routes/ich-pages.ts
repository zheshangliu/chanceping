...
import { buildOpportunityV2Display, filterOpportunityV2Radar, opportunityV2LiveStatus, readOpportunityV2Pool, readOpportunityV2Sources, readOpportunityV2Translations, type OpportunityV2, type OpportunityV2Translation } from "../../opportunity-v2";
...
function v2Card(item: OpportunityV2, index: number, translations: OpportunityV2Translation[]): string {
  ...
  const original = display.original_title ? `<details class="ich-original"><summary>原名</summary><span>${escapeHtml(display.original_title)}</span></details>` : "";
...
function parseV2PageQuery(raw: Record<string, string>): { q: string; category: string; region: string; status: string; sort: string; direction: string; work_format: string; event_region: string; source_id: string; page: number } {
  ...
  return { q: (raw.q ?? "").trim().slice(0, 100), category: raw.category ?? "competition", region: raw.region ?? "all", status: raw.status ?? "current", sort: raw.sort ?? "default", direction: raw.direction ?? "", work_format: raw.work_format ?? "", event_region: raw.event_region ?? "", source_id: raw.source_id ?? "", page };
}
...
function memoDate(item: OpportunityV2): string {
  ...
  return new Date(item.deadline).toLocaleDateString("zh-CN", { timeZone: "Asia/Shanghai" });
}
...
function v2MemoPageBody(result: V2IchPageResult): string {
  ...
  const rows = items.map((item, index) => {
    ...
    const directions: Record<string, string> = { ich_innovation: "非遗创新", cultural_creative: "文创设计", craft_arts: "工艺美术", museum_tourism: "文博文旅", integrated_cultural_design: "综合文化设计", aigc_digital: "AIGC / 数字创作" };
    const dimensions = [...(item.directions ?? []).map((key) => directions[key] ?? key), ...(item.work_formats ?? [])].slice(0, 3).join(" · ") || "分类待补充";
    const location = item.event_location || "地点待补充";
...
export function ichPagesRoutes(options: IchReadRouteOptions = {}): Hono {
  ...
  app.get("/opportunities/:slug", (c) => {
    if (options.opportunityV2) {
      ...
      const location = item.event_location || "地点待补充";
      const deadline = item.is_long_term ? "长期开放" : item.deadline ? new Date(item.deadline).toLocaleDateString("zh-CN", { timeZone: "Asia/Shanghai" }) : "截止时间待确认";
      ...
      const detailBody = `<main class="ich-detail"><p class="ich-detail-kicker">ChancePing · 纸本地域目录 / ${escapeHtml(categoryLabels[item.category] ?? item.category)}</p><span class="ich-status">${escapeHtml(v2StatusLabel(item))}</span><h1>${escapeHtml(display.title)}</h1>${display.original_title ? `<details class="ich-original"><summary>查看原名</summary><p>${escapeHtml(display.original_title)}</p></details>` : ""}<p class="ich-detail-lede">${escapeHtml(display.summary)}</p><div class="ich-detail-layout"><section class="ich-detail-main"><h2>赛事信息</h2><p>方向：${escapeHtml((item.directions ?? []).map((key) => directions[key] ?? key).join(" · ") || "待补充")}<br>作品形式：${escapeHtml((item.work_formats ?? []).map((key) => formats[key] ?? key).join(" · ") || "待补充")}<br>参赛范围：${escapeHtml(item.participation_scope && item.participation_scope !== "unspecified" ? item.participation_scope : "待确认")}<br>参与方式：${escapeHtml(item.participation_mode && item.participation_mode !== "unspecified" ? item.participation_mode : "待确认")}</p><h2>报名与材料</h2><p>${item.application_url ? `<a rel="nofollow noopener" href="${escapeHtml(item.application_url)}">报名入口</a>` : "来源数据尚未提供独立报名入口。请打开来源原文查看提交材料、格式、费用及资格要求。"}</p>${display.original_summary ? `<details class="ich-original"><summary>查看来源摘要</summary><p>${escapeHtml(display.original_summary)}</p></details>` : ""}<div class="ich-source-box"><h2>来源原文</h2><ul>${sourceLinks}</ul><p><a rel="nofollow noopener" href="${escapeHtml(item.detail_url || item.source_url)}">打开赛事来源页面 ↗</a></p></div></section><aside class="ich-detail-aside"><h2>关键节点</h2><dl><dt>截止时间</dt><dd>${escapeHtml(deadline)}</dd><dt>赛事所在地</dt><dd>${escapeHtml(location)}</dd><dt>主办方</dt><dd>${escapeHtml(item.organizer || "来源未提供")}</dd><dt>发现来源</dt><dd>${escapeHtml(sources.map((source) => source.name).join("、") || item.source_name)}</dd></dl></aside></div></main>`;
