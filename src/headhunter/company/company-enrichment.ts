...
export interface TikHubCompanyProfile {
...
export interface CompanyProfileProvider {
  provider: "tikhub";
  getCompanyProfile(company: Company): Promise<TikHubCompanyProfile>;
}
...
interface CacheEntry {
  ...
  fetched_at: string;
  expires_at: string;
  profile: TikHubCompanyProfile;
}

export interface CompanyEnrichmentResult {
  status: "enriched" | "cached" | "unavailable";
  company_id: string;
  profile: TikHubCompanyProfile | null;
  cost: number | null;
...
export async function enrichCompany(company: Company, provider: CompanyProfileProvider | null, options: EnrichmentOptions = {}): Promise<CompanyEnrichmentResult> {
  if (!provider || !company.linkedin_company_url) return { status: "unavailable", company_id: company.company_id, profile: null, cost: null, cost_status: "unknown", reason: "TikHub Company Profile provider or LinkedIn URL unavailable" };
