...
export async function runHeadHunterWeeklyPipeline(options: WeeklyPipelineOptions = {}): Promise<WeeklyPipelineResult> {
  ...
  const stageMetrics: Record<string, number> = { candidate_url_count: uniqueCandidates.length, company_candidate_count: resolutions.length, company_resolved_count: verifiedCompanies.length, company_review_count: resolutions.filter((item) => item.status === "NEEDS_REVIEW" || item.status === "CONFLICT").length, signal_count: signals.length, job_count: jobs.length, person_candidate_count: people.length, verified_person_count: people.filter((item) => item.employment_status === "verified_current").length, contact_count: contacts.length, contact_gate_pass_count: contacts.filter((item) => item.public_verified && item.professional).length, need_count: needCount, lead_count: radar.leads.length, a_count: radar.leads.filter((item) => item.lead_pool === "A_ACTIONABLE").length, b_count: radar.leads.filter((item) => item.lead_pool === "B_ENRICHMENT").length };
