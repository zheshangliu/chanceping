...
export function leadRoutes(context: HeadHunterApiContext): Hono {
  const app = new Hono();
  app.get("/a", async (c) => { const guard = requireAdmin(c, context); if (guard) return guard; return c.json(await hydrateLeads(await context.stores.leads.listByPool("A_ACTIONABLE"), context)); });
  app.get("/b", async (c) => { const guard = requireAdmin(c, context); if (guard) return guard; return c.json(await hydrateLeads(await context.stores.leads.listByPool("B_ENRICHMENT"), context)); });
