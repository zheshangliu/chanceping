...
`/ich/admin` 在原有机会审核与来源提交队列上增加受保护的运营总览：
...
访问统计由服务端只读聚合 Nginx combined access log。接口不返回原始日志、IP、User-Agent
或访客级记录；排除 `/ich/admin`、静态文件、错误响应和常见机器人。独立访客按 IP 与浏览器
标识组合估算，不等同于真实自然人。默认日志路径为 `/var/log/nginx/access.log`，可通过
`CHANCEPING_ICH_ACCESS_LOG_PATH` 覆盖。日志不存在或不可读时后台显示不可用状态，但不得影响
机会与来源审核功能。
