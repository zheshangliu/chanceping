...
- 门禁：**pass**
- 正式机会库：119 条（动态当前 60 条，历史 52 条）
- DS5 快照漂移：当前 60 → 动态 60（差值 0）
- 待复核队列：107 条
- 来源注册：31 个；DS7 流程 31 条
- DS6：3 天周期，4 次账本运行，正式库写入 **false**
- DS8：pass_with_followups；重复主来源 0 组；正式库写入 **false**
- DS8-R2：动作账本 114/114 条；可访问 84；不可访问 30；需人工处置 114
- 仪表盘正式库写入：**false**
