...
MVP 使用 `MAX_CLARIFICATION_ROUNDS = 2`。每轮只展示 1 个自然问题，并始终允许用户选择 `先按默认理解继续`。达到上限后必须生成画像，把仍不确定的内容写成默认假设。
...
LLM 不得编造已检查来源、官方截止时间、参赛资格、报名费用、联系人、BD 意向、获奖后义务或采购预算。

没有字段级证据时，`EvidenceStatus` 不能是 `confirmed`。模型判断必须显式标记为 `model_judgment`，事实与模型共同参与时标记为 `mixed`。报告只能陈述系统真实检查过的来源和已保存的证据状态。
...
- 用户回答后重新生成画像；达到 2 轮后无论分数如何都进入确认摘要，并列出默认假设。
...
`RadarRequirementSpec` 只保存长期画像、风险策略、评分策略和报告蓝图。它不保存 `OpportunityAssessment`、运行分数、候选统计或本轮搜索日志。
...
日志必须来自程序实际执行，不允许让 LLM 在报告生成阶段事后编造。`openedUrls` 只能记录真实内容抓取或网页打开行为；如果本轮没有实际打开网页或抓取正文，`openedUrls` 必须为空，报告也不能写“已核验官网”。
...
运行统计只能从统一的 `CandidateAccounting` 聚合，不允许报告模板自行计算。

```ts
interface CandidateAccounting {
  runId: string;
  rawCount: number;
...
预期：不直接搜索；最多 2 轮、每轮一个问题；生成单一主体画像并显示默认假设。
...
- 画像驱动 SearchPlan。
- 一个真实搜索 Provider。
- Query、真实打开 URL、来源覆盖、RawCandidate 和 CandidateAccounting 持久化。
