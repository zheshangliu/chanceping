...
function auditRuntime(): Record<string, unknown> | null {
  ...
  const unsafePublicExact = conflicts.filter((item) => Boolean(item.deadline_conflict_unsafe && publicOpportunityV2Deadline(item).deadline)).length;
  ...
  return {
    ...
    public_unsafe_exact_deadline_count: unsafePublicExact,
...
if (runtime && Number(runtime.public_unsafe_exact_deadline_count) > 0) process.exitCode = 1;
