...
assert.equal(allRun.sources.length, 7);
assert.equal(allRun.sources.filter((item) => item.status === "succeeded").length, 7);
assert.equal(allRun.totalCount, 7);
...
assert.equal(repeatedRun.totalCount, 7, "repeating the same official notices must not duplicate public cards");
...
assert.equal(failedSourceRun.totalCount, 7, "a failed source must retain its last successful public card");
...
assert.equal(stagedFeed.sources.length, 7);
