...
assert.ok(registry.sources.length >= 100, "Stage 2 source registry must contain at least 100 core sources");
...
assert.ok(adapterReady.every((source) => ["mct-notices", "cnaf", "ichina", "gmfyg", "gz-culture", "cnacs", "gdmuseum", "ccgp", "gd-culture", "yuexiu-notices", "gdmoa", "unesco-ich", "loewe-craft-prize", "china-design-award", "gba-design", "chuangsaiyun-competition-list"].includes(source.id)), "only audited adapters may be adapter_ready");
assert.ok(adapterReady.length <= 16, "DS12 adapter-ready promotion must remain incremental");
