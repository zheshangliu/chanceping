...
assert.ok(adapterReady.every((source) => ["mct-notices", "cnaf", "ichina", "gmfyg", "gz-culture", "cnacs", "gdmuseum"].includes(source.id)), "only audited adapters may be adapter_ready");
assert.ok(adapterReady.length <= 7, "DS10-B adapter-ready promotion must remain incremental");
