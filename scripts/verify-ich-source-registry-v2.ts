...
assert.ok(adapterReady.every((source) => ["mct-notices", "cnaf", "ichina", "gmfyg", "gz-culture", "cnacs", "gdmuseum", "ccgp", "gd-culture", "yuexiu-notices"].includes(source.id)), "only audited adapters may be adapter_ready");
assert.ok(adapterReady.length <= 10, "DS11-C adapter-ready promotion must remain incremental");
