...
if (file.entries.length !== 157) errors.push(`formal_total ${file.entries.length} != 157`);
if (rawActionable !== 43) errors.push(`raw actionable ${rawActionable} != 43`);
if (directionActionable !== 40) errors.push(`direction-aware actionable ${directionActionable} != 40`);
if (channelActionable !== 8) errors.push(`channel actionable ${channelActionable} != 8`);
if (procurementActionable !== 2) errors.push(`procurement actionable ${procurementActionable} != 2`);
if (hash !== summary.after_sha256) errors.push(`summary hash ${summary.after_sha256} != store hash ${hash}`);
const batch4Scripts = fs.readdirSync(path.join(root, "scripts")).filter((name) => /stage5a-batch4/i.test(name));
