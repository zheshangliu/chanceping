...
async function main(): Promise<void> {
  ...
  check("recorded Business data includes six current verified opportunities", records.filter((item) => lifecycleStatus(item, new Date("2026-07-24T12:00:00+08:00")) !== "historical").length >= 6);
  const app = createApp();
  ...
  check("Guangzhou Business list returns verified records", list.status === 200 && listBody.success === true && (listBody.data?.items?.length ?? 0) >= 5);
  ...
  check("Guangzhou current filter excludes expired records", current.status === 200 && currentBody.data?.total === 4);
  ...
  check("Shaoguan current filter returns two actionable opportunities", shaoguan.status === 200 && shaoguanBody.data?.total === 2);
