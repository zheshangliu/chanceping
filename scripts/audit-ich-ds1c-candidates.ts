...
function isDetailUrl(url: string, discoveryUrl: string): boolean {
  try {
    ...
    return source.pathname !== discovery.pathname && /(?:post_|t\d+_\d+|content\/|\/(?:guide_detail|work_notice_detail|normal_detail|fund_work_detail|policy_detail|newslncb_detail|news_2_details|single_detail|event\/detail)\/\d+(?:\.html)?$)/u.test(source.pathname);
  } catch { return false; }
}
