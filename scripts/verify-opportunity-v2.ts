...
async function main(): Promise<void> {
  ...
  const migrationSeedIds = new Set([
    "proc-uk-fts", "proc-ca-canadabuys", "proc-cn-ccgp", "proc-cn-cib", "proc-global-ocp", "proc-eu-ted", "proc-wb",
    "proc-cn-gzsun", "proc-cn-csg", "proc-cn-gz-wglj", "proc-uk-contracts-finder", "cnaf-guides", "een-partnering",
  ]);
  fs.writeFileSync(sourcesPath, JSON.stringify({ ...persistedFixture, sources: persistedFixture.sources.filter((source) => !migrationSeedIds.has(source.id)) }));
  ...
  const fixtureBySource: Record<string, string> = {
    ...
    "https://culture360.asef.org/opportunities/": `<html><body><a href="https://culture360.asef.org/opportunities/loewe-foundation-2027-craft-prize">Loewe Foundation 2027 Craft Prize</a></body></html>`,
