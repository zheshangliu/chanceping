...
async function main(): Promise<void> {
  ...
  const providerLines = ["# Stage5-A.5 Provider Contribution", "", "只统计实际调用；未配置 Provider 的 queries/raw_results 均为 0。", "", "| provider | status | queries | raw_results | errors |", "| --- | --- | ---: | ---: | ---: |", ...providerLogs.map((row) => `| ${row.provider} | ${row.status} | ${row.queries} | ${row.raw_results} | ${row.errors} |`), "", `- provider_search_attempted: ${providerSearchAttempted}`, `- provider_queries: ${providerQueries}`, `- provider_raw_results: ${totalProviderRawResults}`, ""];
