...
async function main(): Promise<void> {
  assert.equal(WELFARE_SHADOW_SOURCES.length, 6, "six verified sources have moved into the public adapter batch");
  assert.equal(new Set(WELFARE_SHADOW_SOURCES.map((source) => source.code)).size, 6, "shadow source codes must be unique");
  ...
  assert.equal(summary.sources.filter((source) => source.status === "succeeded").length, 5);
