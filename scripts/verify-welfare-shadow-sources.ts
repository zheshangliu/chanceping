...
async function main(): Promise<void> {
  assert.equal(WELFARE_SHADOW_SOURCES.length, 23, "three restricted POCs plus the 20-source second batch must stay isolated from the public feed");
  assert.equal(new Set(WELFARE_SHADOW_SOURCES.map((source) => source.code)).size, 23, "shadow source codes must be unique");
  ...
  assert.equal(summary.sources.length, 23);
  assert.equal(summary.sources.find((source) => source.sourceCode === "OFF-N-002")?.status, "restricted");
  assert.equal(summary.sources.filter((source) => source.status === "succeeded").length, 21);
