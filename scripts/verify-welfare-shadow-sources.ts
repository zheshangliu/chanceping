...
async function main(): Promise<void> {
  assert.equal(WELFARE_SHADOW_SOURCES.length, 5, "seven verified sources have moved into the public adapter batch");
  assert.equal(new Set(WELFARE_SHADOW_SOURCES.map((source) => source.code)).size, 5, "shadow source codes must be unique");
  ...
  assert.equal(summary.sources.length, 5);
  assert.equal(summary.sources.find((source) => source.sourceCode === "OFF-N-002")?.status, "restricted");
  assert.equal(summary.sources.filter((source) => source.status === "succeeded").length, 4);
