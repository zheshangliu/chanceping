...
async function run() {
  ...
  check("sidebar supports archiving custom radar windows", heroChatJs.includes("archiveHeroRadarWindow") && heroChatJs.includes("data-action=\"confirm-window-archive\""));
  check("sidebar window actions use in-app modal instead of browser prompts", heroChatJs.includes("openRenameWindowModal") && heroChatJs.includes("openArchiveWindowModal") && !heroChatJs.includes("window.prompt?.") && !heroChatJs.includes("window.confirm?."));
  check("sample room is protected from sidebar archive action", heroChatJs.includes("sample room cannot be archived") || heroChatJs.includes("样板间不能删除"));
