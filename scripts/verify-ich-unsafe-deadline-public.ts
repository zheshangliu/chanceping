...
const requestedRuntime = process.env.ICH_UNSAFE_DEADLINE_RUNTIME;
let runtime = requestedRuntime ?? "/tmp/chanceping-runtime-post-deploy-20260910";
let runtimeSource = requestedRuntime || fs.existsSync(`${runtime}/opportunities.json`) ? "saved-failed-deploy-runtime" : "minimal-regression-fixture";
if (!fs.existsSync(`${runtime}/opportunities.json`)) {
  runtime = fs.mkdtempSync(path.join(os.tmpdir(), "chanceping-unsafe-deadline-runtime-"));
  fs.writeFileSync(`${runtime}/opportunities.json`, JSON.stringify({ schema_version: "chanceping-opportunity-v2.v1", updated_at: "2026-09-10T00:00:00.000Z", opportunities: [unsafeFixture] }));
  fs.writeFileSync(`${runtime}/sources.json`, JSON.stringify({ schema_version: "chanceping-opportunity-v2.sources.v1", updated_at: "2026-09-10T00:00:00.000Z", sources: [fixtureSource] }));
  fs.writeFileSync(`${runtime}/source-health.json`, JSON.stringify({ schema_version: "chanceping-opportunity-v2.source-health.v1", updated_at: "2026-09-10T00:00:00.000Z", sources: [] }));
  fs.writeFileSync(`${runtime}/translations.json`, JSON.stringify({ schema_version: "chanceping-opportunity-v2.translation.v1", updated_at: "2026-09-10T00:00:00.000Z", translations: [] }));
  fs.writeFileSync(`${runtime}/scheduler.json`, JSON.stringify({ interval_hours: 72 }));
}
