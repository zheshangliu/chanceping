...
async function runPublicAiEventsTests(): Promise<void> {
  ...
  const api = await apiGet("/api/public/ai-events?status=current&page=1&page_size=12");
  const apiBody = api.data as {
    ...
    data?: {
      ...
      stats?: {
        ...
        page?: number;
        pageSize?: number;
        categoryFacets?: Array<{ id?: string; count?: number }>;
      };
    };
  };
  const serialized = JSON.stringify(api.data ?? {});
  logPublicCheck("GET /api/public/ai-events current page returns 200", api.status === 200, `status=${api.status}`);
  ...
  logPublicCheck("public API paginates page 1", apiBody.data?.stats?.page === 1 && apiBody.data?.stats?.pageSize === 12 && (apiBody.data?.items?.length ?? 99) <= 12, serialized.slice(0, 160));
  logPublicCheck("public API tracks 30+ total cards", (apiBody.data?.stats?.totalCount ?? 0) >= 30, serialized.slice(0, 160));
  logPublicCheck("public API returns 15+ current displayable cards", (apiBody.data?.stats?.displayableCount ?? 0) >= 15, serialized.slice(0, 160));
  ...
  logPublicCheck("public API exposes primary category field", /primaryCategory/i.test(serialized), serialized.slice(0, 160));
  logPublicCheck("public API exposes category facets", Array.isArray(apiBody.data?.stats?.categoryFacets) && apiBody.data.stats.categoryFacets.length > 0, serialized.slice(0, 160));
  logPublicCheck("public API exposes event mode field", /eventModeLabel/i.test(serialized), serialized.slice(0, 160));
  ...
  logPublicCheck("public API does not push needs-review wording to users", !/待复核|needs_review|Needs review|review required/i.test(serialized), serialized.slice(0, 160));
  const historicalApi = await apiGet("/api/public/ai-events?status=historical&page=1&page_size=12");
  const historicalSerialized = JSON.stringify(historicalApi.data ?? {});
  logPublicCheck("public API historical page returns 200", historicalApi.status === 200, `status=${historicalApi.status}`);
  logPublicCheck("public API historical page succeeds", (historicalApi.data as { success?: boolean })?.success === true, historicalSerialized.slice(0, 160));
  const categoryApi = await apiGet("/api/public/ai-events?status=current&category=ai_agent&page=1&page_size=12");
  const categoryBody = categoryApi.data as { success?: boolean; data?: { items?: Array<{ primaryCategory?: { id?: string }; categoryTags?: Array<{ id?: string }> }> } };
  const categorySerialized = JSON.stringify(categoryApi.data ?? {});
