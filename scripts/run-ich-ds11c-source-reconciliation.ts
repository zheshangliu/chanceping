...
const sourceIds = ["ccgp", "gd-culture", "yuexiu-notices"];
...
const outputPath = path.resolve("docs/ich/DS11-C-来源状态统一审计_V1.0.json");
const reportPath = path.resolve("docs/ich/DS11-C-来源状态统一审计报告_V1.0.md");
...
const sources = sourceIds.map((sourceId) => {
  ...
  return { source_id: sourceId, before_status: source?.operational_status ?? null, adapter_samples: adapterSampleCount, ds1c_candidates: candidates.length, ds1c_blocked: candidates.filter((item) => item.admission !== "candidate_review").length, ds2_status: ds2Status?.status ?? null, ds2_candidates: ds2CandidateCount, ds3_assessments: assessments.length, ds3_approved: assessments.filter((item) => item.decision === "approved").length, ds3_rejected: assessments.filter((item) => item.decision === "reject").length, promotion_eligible: Boolean(source && adapter && adapterSampleCount >= 3 && ds2Status?.status === "completed" && ds2CandidateCount >= 3 && candidates.length >= 3 && candidates.every((item) => item.admission === "candidate_review") && assessments.length >= 3 && assessments.every((item) => item.formal_publish_blocked === true)) };
});
...
fs.writeFileSync(reportPath, `# DS11-C 来源状态统一审计报告 V1.0\n\n- 来源：${sourceIds.join("、")}\n- 只读：**true**\n- 正式库写入：**false**\n- 晋级决定：**${audit.promotion_decision}**\n- 门禁：**${audit.gate}**\n\n${sourceLines}\n\n只有注册表状态与工作流模式一致后，来源才可进入下一轮自动采集观察；候选仍不得自动进入正式机会库。\n\n机器记录：[DS11-C-来源状态统一审计_V1.0.json](./DS11-C-来源状态统一审计_V1.0.json)。\n`, "utf8");
